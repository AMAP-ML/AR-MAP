...
class ScriptArguments:
    ...
    # Legacy parameters (might be unused, kept for compatibility)
...
def _llm_worker_run(args):
    ...
    try:
        ...
        # Using use_tqdm=True to show progress bar for child processes
...
async def main():
    ...
    # --- Setup Multi-GPU Environment Variables ---
    ...
    # --- Data Loading ---
    sft_ds = load_jsonl_dataset('your_path/sft_arena_baseline.jsonl')
    ...
    #################################################################
    # Generation Part: Using JetEngine for generation
    #################################################################
    cprint("\n" + "="*50, "yellow")
    ...
    #################################################################
    # End of Generation: generated_responses now contains all texts
    #################################################################

    # Prepare evaluation queries
    cprint("\n" + "="*50, "yellow"); cprint("Preparing evaluation queries...", "yellow"); cprint("="*50, "yellow")
    eval_queries = []
    avg_len = 0
    ...
    # Call API
    cprint("\n" + "="*50, "yellow"); cprint("Starting evaluation API calls...", "yellow"); cprint("="*50, "yellow")
    endpoint = "https://ai-fcs0515ai378871354499.services.ai.azure.com/openai/v1/"
    ...
    # ===================== Statistic results and save/print details =====================
    cprint("\n" + "="*50, "yellow"); cprint("Calculating evaluation statistics and saving details...", "yellow"); cprint("="*50, "yellow")
    ...
    for i, res in enumerate(api_results):
        ...
        # Print valid judge result every 20 samples
    ...
    print(f'❌ Formatting Errors / API Failures: {err_cnt}')
