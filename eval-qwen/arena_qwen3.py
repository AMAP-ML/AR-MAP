...
async def main():
    ...
    ds = load_jsonl_direct('your_path/arena-hard.jsonl')
    ...
    if script_args.use_lora:
        print(f'使用LoRA路径: {script_args.lora_path}')
    ...
    print(f"✅ 生成的输出（包括原始和清理后版本）已保存到 {output_filename}")
    ...
    print("最终评估结果:")
    print(f"{'='*50}")
    print(f'总查询数: {total_evaluated}')
    print(f'有效回复数: {valid_results_count}')
    print(f'❌ 评估格式错误/API失败数量: {err_cnt}')

    if valid_results_count > 0:
        win_rate = win_cnt / valid_results_count
        # ✅ 修改：这里的平均长度已经是清理后回复的长度
        print(f'✅ 胜率 (基于有效回复): {win_rate:.4f} ({win_cnt}/{valid_results_count})')
        print(f'📊 平均长度 (清理后): {avg_len / len(questions):.2f}')
    else:
        print("没有获得任何有效的评估结果。")
    print(f"{'='*50}\n")
